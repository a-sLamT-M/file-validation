...
impl FileValidationActions {
    ...
    pub fn export(&mut self, folder_path: &Option<String>) -> std::io::Result<String>  {
        ...
        self.hash_gen.save_to_file(None)?;
        Ok(String::from(format!("File saved at {}.", folder_path.clone().unwrap())))
    }

    pub fn import(&mut self, folder_path: &Option<String>, file_path: &str) -> std::io::Result<Vec<&FileHash>> {
        self.init_hash_gen(folder_path)?;
        self.other_hash_gen.load(file_path)?;
